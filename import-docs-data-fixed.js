  ...
  try {
    ...
    if (error.response?.status === 409) {
      ...
      try {
        ...
        if (channels.data.sales_channels && channels.data.sales_channels.length > 0) {
          return channels.data.sales_channels[0].id
        }
      } catch (listError) {
        console.log('⚠️ Could not list existing sales channels, will skip channel linking')
      }
      return null
    }
    throw error
      ...
      try {
        ...
        if (salesChannelId) {
          try {
            ...
            console.log(`⚠️ Failed to link product to sales channel: ${error.response?.data?.message || error.message}`)
          }
        }
        ...
        console.error(`❌ Failed to create product ${product['Product name']}:`, error.response?.data?.message || error.message)
      }
    }
  }
